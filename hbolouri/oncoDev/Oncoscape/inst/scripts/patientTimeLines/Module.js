...
//     var TimeLineSelectionButton;
     ...
     var GreyOutPts = ["TCGA.02.0055", "TCGA.06.0169", "TCGA.02.0099", "TCGA.28.5211", "TCGA.02.0023", "TCGA.76.6286", "TCGA-14-1459", "TCGA.02.0290", "TCGA.06.0124", "TCGA.12.0819", "TCGA.27.2528", "TCGA.27.1837"]  
       ...
       function initializeUI(){
        ...
        TimeLinesendSelectionMenu = $("#TimeLinesendSelectiontoModuleButton")
        TimeLinesendSelectionMenu.change(sendToModuleChanged);
        TimeLinesendSelectionMenu.empty();
        
        TimeLinesendSelectionMenu.append("<option>Send Selection to:</option>")
        ...
        for(var i=0;i< ModuleNames.length; i++){
           ...
           if(SendToModule !== ThisModuleName){
              ...
              TimeLinesendSelectionMenu.append(optionMarkup);
           }
        }  
...
//       	console.log("===== resizing Timeline window")
          TimeLineDisplay.width($(window).width() * 0.95);
           TimeLineDisplay.height($(window).height() * 0.90);
    ...
    //----------------------------------------------------------------------------------------------------
     function sendToModuleChanged() {
       ...
       ModuleName = TimeLinesendSelectionMenu.val()
       timelineSelectionToModule(ModuleName)
       TimeLinesendSelectionMenu.val("Send Selection to:")
    } // sendToModuleChanged
...
//--------------------------------------------------------------------------------------------
  function timelineD3PlotBrushReader(){
   ...
   function handlePatientIDs(msg){
      ...
      console.log(msg)
      var tabIndex = $('#tabs a[href="#patientTimeLinesDiv"]').parent().index();
         ...
         console.log("==== DisplayPatientTimeLine  Events");     
          ...
          FormatDate = d3.time.JAVASCRIPT_FORMAT ("%x");
            ...
            Events.forEach(function(d) {
               ...
               if(d.Type === "[Not Available]" ) d.Type = "NA"
               d.disabled = false;
              if(ShowEvents){
               ...
               if(SidePlotEvent === "--"){     return;     
                    ...
                    if(SidePlotEvent === "Survival"){
                       PatientOrderBy.forEach(function(d){
                            if(EventsByID.get(d.ID).has("Status") & EventsByID.get(d.ID).get("Status")[0].Type !== "Dead")
                                d.width = 0;
                       })
...
//			var HorizLine = svg.append("g").attr("class", "rect")
          ...
          dispatch.on("DisplayPatients.TimeLineDisplay",  function(){
               ...
               var EventOffset = d3.map({"Radiation": (1.5*PatientHeight/y.domain()[1]), "Chemo": (-1.5*PatientHeight/y.domain()[1])})
