...
var TimeLineModule = (function () {
   ...
   broadcastSelection = function(){
      ...
      for(var i=0; i < Events.length; i++){
         event = Events[i];
         if(event.PatientNum >= x1 & event.PatientNum )
            ids.push(event.PatientId[0]);
         } // for i
      if(ids.length > 0)
...
//	   $("#tabs").tabs( "option", "active", 7);
	...
	dispatch.on("LoadOptions.AllDisplays", function(){
		...
		console.log("======== DisplayPatients.Menu")
	      ...
	      OrderByMenu.selectAll("option")
	  		...
	  		AlignByMenu.selectAll("option")
				.data(d3.merge([["None"],EventTypes.keys()]))
				.enter()
					.append("option")
					.attr("value", function(d){return d})
					.text(function(d) { return d})
	...
	OrderEvents = function(){
		...
		if(OrderBy === "Survival"){  PatientOrderBy = getDateDiff("Diagnosis", "Status", "")
