# test_TCGAPatientHistoryProvider.R
#----------------------------------------------------------------------------------------------------
library(RUnit)
library(Oncoscape)
library(data.table)
...
test_constructor <- function()
{
    ...
    path <- "tcga://../extdata/tcgaGBM/patientHistory"
    cdp <- PatientHistoryProvider(path);
    checkTrue(is(cdp, "CaisisDataProvider"))
    checkTrue(is(cdp, "LocalFileCaisisDataProvider"))
...
test_getPatientsAndEventIDs <- function()
{
    ...
    path <- "file://../explorations/caisis/Caisis_BrainTables_5-28-14";
    cdp <- CaisisDataProvider(path);
...
{
       # stop date missing, fix by assigning start date
    patients <- "FC5PKZ244GQOB098PB2IH2C7X33XO1OT765X"
    eventName <- "MedicalTherapy"
    ...
    x2 <- x2[which(!is.na(x2))]
    x3 <- assignPatientNumbersToTimelinesEvent(x2)
    checkEquals(x3[[1]]$date[1], x3[[1]]$date[2])
